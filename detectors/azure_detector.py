...
class AzureObjectDetector(ObjectDetector):
    ...
    def detect(self, image):
        ...
        prediction_outputs = self.process(inputs)
        return self.postprocess(prediction_outputs, image.shape)
    ...
    def postprocess(self, prediction_outputs, frame_size):
        ...
        return [{'bbox': (int(selected_boxes[i][0] * frame_size[1]),
                          int(selected_boxes[i][1] * frame_size[0]),
                          int((selected_boxes[i][0] + selected_boxes[i][2]) * frame_size[1]),
                          int((selected_boxes[i][1] + selected_boxes[i][3]) * frame_size[0])),
                 'score': round(float(selected_probs[i]), 8),
                 'class': self.labels[selected_classes[i]]}
...
class TFLiteAzureObjectDetector(AzureObjectDetector):
    """Object Detection class for TensorFlow Lite"""

    def __init__(self, model_filepath, labels_filepath):
        with open(labels_filepath, 'r') as f:
            labels = [l.strip() for l in f.readlines()]
        super(TFLiteAzureObjectDetector, self).__init__(labels)
        self.interpreter = tflite.Interpreter(model_path=model_filepath, experimental_delegates=[tflite.load_delegate(_EDGETPU_SHARED_LIB, {})])
